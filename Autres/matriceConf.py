...
result_file = "res_lbph_test.txt"

# Seuils à tester
thresholds = [1.25, 1.45, 1.65, 1.85]
...
for line in lines:
    # Exemple de ligne : "Adam_1.jpg -> Adam : 1.21"
    parts = line.strip().split("->")
    if len(parts) != 2:
        continue  # Ignorer les lignes mal formées

    left_part, right_part = parts
    filename = left_part.strip()
    
    # Traiter la partie droite (label et distance)
    label_distance = right_part.strip().split(":")
    if len(label_distance) != 2:
        continue  # Ignorer les lignes mal formées

    label = label_distance[0].strip()
    try:
        distance = float(label_distance[1].strip())
    except ValueError:
        continue  # Ignorer les lignes avec des distances invalides
