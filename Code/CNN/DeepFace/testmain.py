...
class CameraApp(App):
    ...
    def take_screenshot(self, instance):
        # Capture d'écran basée sur l'image actuelle (sans détection des visages)
        texture = self.camera.texture
        if texture:
            ...
            # Convertir l'image de RGBA à BGR pour la détection
            img_bgr = cv2.cvtColor(img, cv2.COLOR_RGBA2BGR)

            # Détection des visages sur cette image
            try:
                # Utiliser DeepFace pour extraire les visages de l'image capturée
            ...
            # Dessiner les rectangles et ajouter les annotations sur l'image capturée
            for face in faces_data:
                ...
                # Ajouter un label "Reconnaissance en cours" sous le rectangle
                label_text = "Reconnaissance en cours"
                font_scale = 0.5
                thickness = 1
                font = cv2.FONT_HERSHEY_SIMPLEX
            ...
            # Convertir l'image annotée de BGR à RGBA pour Kivy
            img_rgba = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGBA)

            # Enregistrer l'image annotée avec visages détectés
            cv2.imwrite("screenshot_with_faces.png", img_bgr)  # Enregistrer en BGR
            print("Capture d'écran avec visages enregistrée sous 'screenshot_with_faces.png'")
